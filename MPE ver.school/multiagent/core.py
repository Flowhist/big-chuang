...
class Movmark(Entity):
    ...
    def control(self):
        ...
        if abs(self.direction) == 1:
            if abs(self.state.p_pos[0]) > 1:
                self.direction = -self.direction
        if abs(self.direction) == 2:
            if abs(self.state.p_pos[1]) > 1:
                self.direction = -self.direction
        if abs(self.direction) == 1:  # 左右移动
            if self.direction > 0:  # 右
                self.state.p_vel[0] = 2
                self.state.p_vel[1] = 0
                self.state.p_pos += self.state.p_vel * k
            else:   # 左
                self.state.p_vel[0] = -2
                self.state.p_vel[1] = 0
                self.state.p_pos += self.state.p_vel * k
        if abs(self.direction) == 2:  # 上下移动
            if self.direction > 0:  # 上
                self.state.p_vel[0] = 0
                self.state.p_vel[1] = 2
                self.state.p_pos += self.state.p_vel * k
            else:   # 下
                self.state.p_vel[0] = 0
                self.state.p_vel[1] = -2
...
class World(object):
    ...
    @property
    def entities(self):
        return self.agents + self.landmarks + self.movmarks + self.terminals
